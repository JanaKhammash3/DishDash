...
class _MealPlannerScreenState extends State<MealPlannerScreen> {
  ...
  Future<void> markMealAsDoneBackend(
    ...
    final url = Uri.parse('http://192.168.68.61:3000/api/mealplans/mark-done');
    await http.put(
      url,
  ...
  Future<void> rateRecipe(String recipeId, int rating) async {
    await http.patch(
      Uri.parse('http://192.168.68.61:3000/api/recipes/rate/$recipeId'),
      headers: {'Content-Type': 'application/json'},
  ...
  void _updateMealDate(int index) async {
    final meal = plannedMeals[index];

    // 🛠 Ensure date is parsed correctly from string if needed
    final DateTime oldDate =
        meal['date'] is String ? DateTime.parse(meal['date']) : meal['date'];
    ...
    if (newDate != null && newDate != oldDate) {
      ...
      final oldDateStr = oldDate.toIso8601String().split('T')[0];
      final newDateStr = newDate.toIso8601String().split('T')[0];
      final recipeId = meal['recipe']['_id'];

      final url = Uri.parse(
        'http://192.168.68.61:3000/api/mealplans/$planId/update-date',
      );
      ...
      if (response.statusCode == 200) {
        setState(() {
          plannedMeals[index]['date'] = newDate;
          plannedMeals = List.from(plannedMeals);
        });
        await saveMealPlan();
        print('✅ Meal date updated');
      } else {
        print('❌ Failed to update meal date');
      }
    }
  }
  ...
  Future<void> loadMealsFromBackend() async {
    final res = await http.get(
      Uri.parse(
        'http://192.168.68.61:3000/api/mealplans/user/${widget.userId}',
      ),
    );
                                  ...
                                  if (planResponse.statusCode == 200) {
                                    ...
                                    if (plan == null) {
                                      ...
                                      final createResponse = await http.post(
                                        Uri.parse(
                                          'http://192.168.68.61:3000/api/mealplans',
                                        ),
                                        headers: {
                                    ...
                                    if (plan != null) {
                                      ...
                                      final addResponse = await http.put(
                                        Uri.parse(
                                          'http://192.168.68.61:3000/api/mealplans/$planId/add-recipe',
                                        ),
                                        headers: {
                                          'Content-Type': 'application/json',
                                        },
  ...
  void _removeMeal(int index) async {
    ...
    final url = Uri.parse(
      'http://192.168.68.61:3000/api/mealplans/$planId/remove-recipe',
    );
                    ...
                    children: [
                      IconButton(
                        ...
                        onPressed: () async {
                          ...
                          final url = Uri.parse(
                            'http://192.168.68.61:3000/api/mealplans/${willBeDone ? 'mark-done' : 'mark-undone'}',
                          );
                      ...
                      IconButton(
                        ...
                        onPressed: () => _updateMealDate(index),
                      ),
                      IconButton(
