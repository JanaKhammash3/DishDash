...
class _RecommendationsWidgetState extends State<RecommendationsWidget> {
  ...
  Future<void> loadSavedRecipes() async {
    final url = Uri.parse(
      'http://192.168.0.101:3000/api/profile/${widget.userId}',
    );
  ...
  Future<void> saveRecipe(String recipeId) async {
    final url = Uri.parse(
      'http://192.168.0.101:3000/api/users/${widget.userId}/saveRecipe',
    );
    await http.post(
      url,
  ...
  Future<void> unsaveRecipe(String recipeId) async {
    final url = Uri.parse(
      'http://192.168.0.101:3000/api/users/${widget.userId}/unsaveRecipe',
    );
    await http.post(
      url,
      headers: {'Content-Type': 'application/json'},
  ...
  Future<void> fetchRecommendations() async {
    try {
      final url = Uri.parse(
        'http://192.168.0.101:3000/api/users/${widget.userId}/recommendations',
      );
            ...
            mealTimeRaw.map<Map<String, dynamic>>((recipe) {
              ...
              final imagePath =
                  isBase64 || isNetwork
                      ? rawImage
                      : rawImage.isNotEmpty
                      ? 'http://192.168.0.101:3000/images/$rawImage'
