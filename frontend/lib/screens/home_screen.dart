...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Future<void> fetchUnreadCount() async {
    final res = await http.get(
      Uri.parse(
        'http://192.168.0.101:3000/api/notifications/${widget.userId}/unread-count',
      ),
    );
          ...
          data.map((recipe) {
            ...
            final imagePath =
                (image != null && image.isNotEmpty)
                    ? 'http://192.168.0.101:3000/images/$image'
  ...
  Future<void> fetchUserProfile() async {
    final url = Uri.parse('http://192.168.0.101:3000/api/profile/$userId');
  ...
  Future<void> fetchRandomRecipes() async {
    final url = Uri.parse('http://192.168.0.101:3000/api/recipes');
    final response = await http.get(url);
    if (response.statusCode == 200) {
  ...
  Future<void> _saveRecipe(String recipeId) async {
    final url = Uri.parse(
      'http://192.168.0.101:3000/api/users/$userId/saveRecipe',
    );
  ...
  Future<void> _unsaveRecipe(String recipeId) async {
    final url = Uri.parse(
      'http://192.168.0.101:3000/api/users/$userId/unsaveRecipe',
    );
    final response = await http.post(
      url,
      headers: {'Content-Type': 'application/json'},
  ...
  Future<void> fetchPopularRecipes({String? category}) async {
    String baseUrl = 'http://192.168.0.101:3000/api/recipes/filter';
    Uri url;
            ...
            (index) {
              ...
              final imagePath =
                  rawPath.startsWith('/images/')
                      ? 'http://192.168.0.101:3000$rawPath'
                      : rawPath;
